...
# from Darboux.darboux_1_20.Function import *
...
class Search(SearchInit):
    ...
    def Enumerate_Init_Activation(self):
        for activationset in self.S_init.values():
            # print("The initial activationset is")
            # print(activationset)
    ...
    def recursive_enumeration(self, activation_set, ind_layer, ind_neuron, count_num):
        if self.isRealActivation(activation_set):
            # print("recursive_enumeration!")
            # print(activation_set)
    ...
    def TestOne(self, S):
        flag = False
        count_equ = 0 
        SSpace = [-2, 2]  # this condition is for Darboux only
    ...
    def plot_neural(self):
        gridsize = 400
        nx = torch.linspace(-10, 10, gridsize)
        ny = torch.linspace(-25, 25, gridsize)
    ...
    def verification(self):
        ...
        for ind, item in enumerate(self.veri_acti):
            ...
            if result.is_success() == False:
                ...
                if result.get_optimal_cost() < 0:
                    # lineqs(cons_W, -cons_r, fig=fig, ax=ax, color=colorlist[cnt], alpha=0.5, s=10, size=(15, 15),
                    #        save=False,
                    #        show=True)
                    # plt.show()
        ...
        t2 = time.time()
        print(f'optimization positive invariance costs', t2 - t1, 'seconds')
        if flag_positive == False:
            print("optimization verification failed! For positive invariance")
        ...
        b = Variable("b", Variable.Real)
        c = Variable("c", Variable.Real)
        variables = np.array([x, y, z, a, b, c])
        for ind, item in enumerate(self.veri_acti):
            f_sat_part = 1
            W_B, r_B, W_o, r_o = LinearExp(self.model, item)
            cons_W, cons_r = RoAMatrix(self.model, S=None, W_B=W_B, r_B=r_B, SSpace=self.case.SSpace)
            ll = len(W_o) - 1
            f_sat_part = np.dot(W_o[ll], self.vectorfield_dreal(variables))[0] > 0
            region_constraint = np.dot(cons_W, variables)
            for i in range(cons_W.shape[0]):
                f_sat_part = And(f_sat_part, region_constraint[i] >= -cons_r[i])
            for i in range(len(variables)):
                f_sat_part = And(f_sat_part, variables[i] <= 1e10)
                f_sat_part = And(f_sat_part, variables[i] >= -1e10)
            ...
            if result != None:
                ...
                break
        t3 = time.time()
        print(f'SMT verification for positive invariance costs', t3 - t2, 'seconds')
        ...
        if included_flag == True:
            ...
            for ind, item in enumerate(self.veri_acti):
                ...
                cons_W, cons_r = RoAMatrix(self.model, S=None, W_B=W_B, r_B=r_B, SSpace=self.case.SSpace)
                ll = len(W_o) - 1
                f_sat_part = 0.04 - x ** 2 - y ** 2 - z ** 2 - a ** 2 - b ** 2 - c ** 2>= 0
                region_constraint = np.dot(cons_W, variables)
                for i in range(cons_W.shape[0]):
                    f_sat_part = And(f_sat_part, region_constraint[i] >= -cons_r[i])
                for i in range(len(variables)):
        ...
        if included_flag:
            ...
            for ind, item in enumerate(self.veri_acti):
                ...
                cons_W, cons_r = RoAMatrix(self.model, S=None, W_B=W_B, r_B=r_B, SSpace=self.case.SSpace)
                ll = len(W_o) - 1
                f_sat_part = (x - 3) ** 2 + (y - 3) ** 2 + (z - 3) ** 2 + (a - 3) ** 2 + (b - 3) ** 2 + (c - 3) ** 2 - 1 <= 0
                region_constraint = np.dot(cons_W, variables)
                for i in range(cons_W.shape[0]):
                    f_sat_part = And(f_sat_part, region_constraint[i] >= -cons_r[i])
                for i in range(len(variables)):
    ...
    def vectorfield_dreal(self, x):
        x0_dot = -x[0] * (1 + x[0] ** 2 + x[1] ** 2 + x[2] ** 2 + x[3] ** 2 + x[4] ** 2 + x[5] ** 2)
        x1_dot = -x[1] * (1 + x[0] ** 2 + x[1] ** 2 + x[2] ** 2 + x[3] ** 2 + x[4] ** 2 + x[5] ** 2)
        x2_dot = -x[2] * (1 + x[0] ** 2 + x[1] ** 2 + x[2] ** 2 + x[3] ** 2 + x[4] ** 2 + x[5] ** 2)
        x3_dot = -x[3] * (1 + x[0] ** 2 + x[1] ** 2 + x[2] ** 2 + x[3] ** 2 + x[4] ** 2 + x[5] ** 2)
        x4_dot = -x[4] * (1 + x[0] ** 2 + x[1] ** 2 + x[2] ** 2 + x[3] ** 2 + x[4] ** 2 + x[5] ** 2)
        x5_dot = -x[5] * (1 + x[0] ** 2 + x[1] ** 2 + x[2] ** 2 + x[3] ** 2 + x[4] ** 2 + x[5] ** 2)
        return np.array([x0_dot, x1_dot, x2_dot, x3_dot, x4_dot, x5_dot])
    ...
    def TestOneTemp(self, S, fig, ax):
        ...
        result2 = solver.Solve(prog2)
        if result1.is_success() and result2.is_success():
            # print("result1 and result2 is successful! in equality constraint!")
            if abs(result1.get_optimal_cost() + result2.get_optimal_cost()) < eps1: 
                if count_equ == 0:
                    W_equ = bdyW
                    W_equ = np.vstack([W_equ, -bdyW[0]])
