...
class ComplexCase(case):
    ...
    def f_x(self, x):
        ...
        x0_dot = -x[0] * (1 + np.sin(x[1]) ** 2 + np.exp(-x[2] ** 2))
        x1_dot = -x[1] * (1 + np.cos(x[2]) ** 2 + np.tanh(x[0]) ** 2)
        x2_dot = -x[2] * (1 + np.log(1 + x[0] ** 2 + x[1] ** 2))
        x_dot = np.vstack([x0_dot, x1_dot, x2_dot])
        return x_dot
