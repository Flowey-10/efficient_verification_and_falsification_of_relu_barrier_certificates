...
class ComplexCase(ControlAffineSystem):
    ...
    def state_limits(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        upper_limit[ComplexCase.X] = 2.0
        upper_limit[ComplexCase.Y] = 2.0
    ...
    def unsafe_mask(self, x):
        ...
        h = x[:, ComplexCase.X] + x[:, ComplexCase.Y] ** 2
    ...
    def _f(self, x: torch.Tensor, params: Scenario):
        ...
        f[:, ComplexCase.X, 0] = -x0_ * (1 + torch.sin(x1_) ** 2 + torch.exp(-x2_ ** 2))
        f[:, ComplexCase.Y, 0] = -x1_ * (1 + torch.cos(x2_) ** 2 + torch.tanh(x0_) ** 2)
        f[:, ComplexCase.Z, 0] = -x2_ * (1 + torch.log(1 + x0_ ** 2 + x1_ ** 2))
