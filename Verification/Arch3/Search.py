...
from verification_of_relu_test.Arch3.prob import vector_field
...
class Search(SearchInit):
    ...
    def TestOne(self, S):
        #has modified the precision from 1e-16 to 1e-7
        # This section has problem
        # print("Start of TestOne")
        # print("S=")
        # print(S)
        # eps1 = 1e-8
        flag = False
    ...
    def plot_neural(self):
        gridsize = 400
        nx = torch.linspace(-2, 2, gridsize)
        ny = torch.linspace(-2, 2, gridsize)
    ...
    def plot_vector(self):
        gridsize = 10
        nx = np.linspace(-2, 2, gridsize)
        ny = np.linspace(-2, 2, gridsize)
    ...
    def vectorfield_dreal(self, x): # x must be a np.array
        x0_dot = x[0] - x[0] ** 3 + x[1] - x[0] * x[1] ** 2
        x1_dot = -x[0] + x[1] - x[0] ** 2 * x[1] - x[1] ** 3
