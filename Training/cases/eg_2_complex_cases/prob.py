...
def cons_unsafe(x):
    # return x[:, 0] == x[:, 0]
    unsafe = (x[:, 0] - 0) * (x[:, 0] - 0) + (x[:, 1] + 0) * (x[:, 1] + 0) + (x[:, 2] + 0) * (x[:, 2] + 0) >= 0.25 + superp.TOL_DATA_GEN # a cylinder
    return unsafe
...
def vector_field(x):
    def f(i, x):
        if i == 1:
            return -x[:, 0] * (1 + torch.sin(x[:, 1]) ** 2 + torch.exp(-x[:, 2] ** 2))  # x[:, 1] stands for x2
        elif i == 2:
            return -x[:, 1] * (1 + torch.cos(x[:, 2]) ** 2 + torch.tanh(x[:, 0]) ** 2)  # x[:, 0] stands for x1
        elif i == 3:
            # return -x[:, 0] - 2 * x[:, 1] - x[:, 2] + x[:, 0] ** 3
            return -x[:, 2] * (1 + torch.log(1 + x[:, 0] ** 2 + x[:, 1] ** 2))
        else:
