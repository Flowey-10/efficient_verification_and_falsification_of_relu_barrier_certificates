...
def cons_unsafe(x):
    return x[:, 0] ** 2 + x[:, 1] ** 2 >= 0.16 + superp.TOL_DATA_GEN # a parabola
...
def vector_field(x):
    ...
    def f(i, x):
        if i == 1:
            return x[:, 0] - x[:, 0] ** 3 + x[:, 1] - x[:, 0] * x[:, 1] ** 2   # x[:, 1] stands for x2
        elif i == 2:
            return -x[:, 0] + x[:, 1] - x[:, 0] ** 2 * x[:, 1] - x[:, 1] ** 3 # x[:, 0] stands for x1
        else:
